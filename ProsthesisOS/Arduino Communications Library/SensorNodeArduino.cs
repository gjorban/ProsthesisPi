    ...
    public sealed class SensorNodeArduino : ArduinoCommsBase
    {
        ...
        protected override void OnTelemetryReceive(string telemetryData)
        {
            ...
            catch (Newtonsoft.Json.JsonException e)
            {
                if (mLogger != null)
                {
                    mLogger.LogMessage(ProsthesisCore.Utility.Logger.LoggerChannels.Arduino, string.Format("AID:{0} failed to parse JSON \"{1}\" because of {2}. Disabling telemetry", mArduinoID, telemetryData, e));
                }
